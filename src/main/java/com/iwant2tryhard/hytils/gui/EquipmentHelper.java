...
public class EquipmentHelper {
    ...
    public void RenderOverlay(TickEvent.RenderTickEvent event) {
        boolean isShowAllKeyDown = false;
        for (KeyBinding key : Hytils.instance.mc.gameSettings.keyBindings) {
            if (key.getKeyDescription().equals("Show scores for all items") && key.getKeyCategory().equals("Equipment Helper")) {
                isShowAllKeyDown = Keyboard.isKeyDown(key.getKeyCode());
            }
        }
                ...
                for (int i = 0; i < chestContainer.inventorySlots.size(); ++i) {
                    ...
                    if (slot.getStack() != null) {
                        if (!isShowAllKeyDown) {
                            ...
                            double maxChestScore = Double.MIN_VALUE;
                            ConcurrentHashMap<Integer, Double> cache1 = new ConcurrentHashMap<>(Hytils.instance.getUtils().chestScoreMap);
                            for (Map.Entry<Integer, Double> mapItem : cache1.entrySet()) {
                                if (mapItem.getValue() > maxChestScore) {
                                    maxChestScore = mapItem.getValue();
                                }
                            }
                            ...
                            double maxLegsScore = Double.MIN_VALUE;
                            ConcurrentHashMap<Integer, Double> cache2 = new ConcurrentHashMap<>(Hytils.instance.getUtils().legsScoreMap);
                            for (Map.Entry<Integer, Double> mapItem : cache2.entrySet()) {
                                if (mapItem.getValue() > maxLegsScore) {
                                    maxLegsScore = mapItem.getValue();
                                }
                            }
                            ...
                            double maxFeetSize = Double.MIN_VALUE;
                            ConcurrentHashMap<Integer, Double> cache3 = new ConcurrentHashMap<>(Hytils.instance.getUtils().feetScoreMap);
                            for (Map.Entry<Integer, Double> mapItem : cache3.entrySet()) {
                                if (mapItem.getValue() > maxFeetSize) {
                                    maxFeetSize = mapItem.getValue();
                                }
                            }
                            ...
                            double maxSwordSize = Double.MIN_VALUE;
                            ConcurrentHashMap<Integer, Double> cache4 = new ConcurrentHashMap<>(Hytils.instance.getUtils().swordScoreMap);
                            for (Map.Entry<Integer, Double> mapItem : cache4.entrySet()) {
                                if (mapItem.getValue() > maxSwordSize) {
                                    maxSwordSize = mapItem.getValue();
                                }
                            }
                            ...
                            double maxBowSize = Double.MIN_VALUE;
                            ConcurrentHashMap<Integer, Double> cache5 = new ConcurrentHashMap<>(Hytils.instance.getUtils().bowScoreMap);
                            for (Map.Entry<Integer, Double> mapItem : cache5.entrySet()) {
                                if (mapItem.getValue() > maxBowSize) {
                                    maxBowSize = mapItem.getValue();
                                }
                            }
                            ...
                            double maxRodSize = Double.MIN_VALUE;
                            ConcurrentHashMap<Integer, Double> cache6 = new ConcurrentHashMap<>(Hytils.instance.getUtils().rodScoreMap);
                            for (Map.Entry<Integer, Double> mapItem : cache6.entrySet()) {
                                if (mapItem.getValue() > maxRodSize) {
                                    maxRodSize = mapItem.getValue();
                                }
                            }
